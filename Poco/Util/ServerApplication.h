...
//
	...
	#define POCO_SERVER_MAIN_TIER1(App) \
	int main(int argc, char** argv)		\
	{									\
		try 							\
		{								\
			...
			for(int i=0;i<argc;i++) \
				__argv[i] = argv[i]; \
			__argv[__argc - 1] = (char*)"--config=/etc/macchina-tier1.properties"; \
	...
	#define POCO_SERVER_MAIN_TIER2(App) \
	int main(int argc, char** argv)		\
	{									\
		try 							\
		{								\
			...
			for(int i=0;i<argc;i++) \
				__argv[i] = argv[i]; \
			__argv[__argc - 1] = (char*)"--config=/etc/macchina-tier2.properties"; \
			return app.run(__argc, __argv);	\
		}								\
		catch (Poco::Exception& exc)	\
		{								\
	...
	#define POCO_SERVER_MAIN_TIER3(App) \
	int main(int argc, char** argv)		\
	{									\
		try 							\
		{								\
			...
			for(int i=0;i<argc;i++) \
				__argv[i] = argv[i]; \
			__argv[__argc - 1] = (char*)"--config=/etc/macchina-third.properties"; \
			return app.run(__argc, __argv);	\
		}								\
		catch (Poco::Exception& exc)	\
		{								\
